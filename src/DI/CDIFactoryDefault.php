    ...
    public function __construct()
    {
       ...
       $this->setShared('session', function() {
        ...
        $this->setShared('db', function() {
            ...
            $db->setOptions(require ANAX_APP_PATH . 'config/mysql.php');
            $db->connect();
            return $db;
        });
        ...
        $this->set('CommentController', function() use ($di) {
            $controller = new \Anax\Comment\CommentController();
            $controller->setDI($di);
            return $controller;
        });

        $this->set('QuestionsController', function() use ($di) {
            $controller = new \Anax\Question\ProjectquestionsController();
            $controller->setDI($di);
            return $controller;
        });

        $this->set('UsersController', function() use ($di) {
            $controller = new \Anax\Users\ProjectusersController();
            $controller->setDI($di);
            return $controller;
        });

        $this->set('SearchController', function() use ($di) {
        $controller = new \Anax\Search\ProjectsearchController();
        $controller->setDI($di);
        return $controller;
    });
